...
6. No error handling around  Image.open . A non-image file with an image-like extension will raise an unhandled  PIL.UnidentifiedImageError  → 500, instead of a clean form/validation error.

7.  backfill_thumbnails  command omits  SummernoteAttachment  — only covers  BoardImage / PictureOfWeek , so any pre-existing inline article images without a thumbnail are never backfilled, despite the model having the same  thumbnail  field shape.

8.  (minor) Double bleach sanitization:  SummernoteTextFormField.to_python  and  SummernoteTextField.to_python  both call  bleach.clean()  — harmless but redundant, worth a comment or removing the model-field duplication.
